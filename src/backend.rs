...
use p2panda_store::sqlite::store::{
    connection_pool, create_database, run_pending_migrations
};
use p2panda_store::{LocalOperationStore, LogStore, MemoryStore};
...
pub type ButtStore = MemoryStore<ButtLogId, ButtExtensions>;
...
impl Backend {
    pub async fn new(private_key: PrivateKey, data_path: String) -> Self {
        ...
        run_pending_migrations(&connection_pool)
            .await
            .expect("database migrations ran");
        let store = ButtStore::new();
